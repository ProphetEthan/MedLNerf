...
class ConvNeRF(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            alpha = self.alpha_conv(h).squeeze(-1)
            feature = self.feature_conv(h)
            h = torch.cat([feature, input_views], dim=1)

            for conv in self.views_conv_layers:
                h = conv(h)
                h = F.relu(h)

            rgb = self.rgb_conv(h).squeeze(-1)
            outputs = torch.cat([rgb, alpha], dim=-1)
        else:
            outputs = self.output_conv(h).squeeze(-1)
